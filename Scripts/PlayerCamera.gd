...
@onready var canvas_item = $LineDrawer as Node2D
...
func drag_housebuild():
	...
	if not draghouse:
		if Input.is_action_just_pressed("builder_lmb") and not dragcam:
			draghouse = true
			mouse_origin = get_viewport().get_mouse_position()
	else:
...
func _process(delta):
	...
	rotation.y = radrot
	position = baseVector * Basis.from_euler(Vector3(0, -radrot, 0))
	position.y = 4
	pass
...
func _on_node_2d_draw():
	if (draghouse):
		...
		canvas_item.draw_line(vec0, vec1, Color.RED, LINE_WIDTH)
		canvas_item.draw_line(vec1, vec2, Color.RED, LINE_WIDTH)
		canvas_item.draw_line(vec2, vec3, Color.RED, LINE_WIDTH)
		canvas_item.draw_line(vec3, vec0, Color.RED, LINE_WIDTH)
	pass # Replace with function body.
